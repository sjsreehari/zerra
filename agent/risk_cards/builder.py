...
class RiskCardBuilder:
    def build(self, *, event: CallEvent, verdict: Verdict, graph_result: GraphScoreResult | None,
        ...
        confidence = min(1.0, max(graph_score, sequence_score, .95 if hard else 0.0))
        owasp, mitre = self._tags(names, pattern, graph_result)
        evidence_parts = [f"call {event.id}"]
        if graph_result:
            evidence_parts.append(f"graph: {graph_result.evidence}")
        if pattern and sequence_result and sequence_result.triggered_pattern:
            evidence_parts.append(f"sequence: {pattern} at call index {sequence_result.triggered_pattern.matched_at_call_index}")
        if trust_result:
            evidence_parts.append(f"trust: {trust_result.evidence}")
        evidence_parts.extend(f"policy {item.policy_name}: {item.reason}" for item in policies)
        ...
        return RiskCard(id=str(uuid5(NAMESPACE_URL, f"{event.id}:{verdict.value}")), identity_id=event.identity_id,
                        ...
                        evidence="; ".join(evidence_parts), timestamp=datetime.now(timezone.utc), factors=factors)
    ...
    def _tags(policy_text: str, pattern: str | None, graph_result: GraphScoreResult | None) -> tuple[str, str]:
        ...
        if "revok" in policy_text:
            return "API2: Broken Authentication", "T1078-adjacent: Valid Accounts"
        if "scope" in policy_text:
            return "API5: Broken Function Level Authorization", "T1078-adjacent: Valid Accounts"
        if "cross-tenant" in policy_text or "cross-tenant" in graph_text:
            return "API1: Broken Object Level Authorization", "T1190-adjacent: Exploit Public-Facing Application"
        if pattern == "credential_probing":
            return "API2: Broken Authentication", "T1110-adjacent: Brute Force"
        if pattern == "enumeration_exfil" or "fan_out" in graph_text or "distinct objects" in graph_text:
            return "API1: Broken Object Level Authorization", "T1190-adjacent: Exploit Public-Facing Application"
        return "API4: Unrestricted Resource Consumption", "T1496-adjacent: Resource Hijacking"
