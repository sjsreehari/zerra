...
func (r DockerRunner) Run(ctx context.Context, plan securityscan.ScanPlan) (securityscan.ScanReport, error) {
	...
	resp, err := r.Client.ContainerCreate(timeoutCtx, &container.Config{
		Image:        r.Image,
		Entrypoint:   strslice.StrSlice{"/scanner"},
		Cmd:          strslice.StrSlice{"--plan-stdin"},
		User:         "65532:65532",
		OpenStdin:    true,
		StdinOnce:    true,
		AttachStdin:  true,
	...
	if _, err = stdcopy.StdCopy(&stdout, &stderr, io.LimitReader(logs, maxOutput+1)); err != nil {
		return securityscan.ScanReport{}, errors.New("could not decode scanner logs")
	}
	if stdout.Len()+stderr.Len() > maxOutput {
...
func parseReport(output []byte) (securityscan.ScanReport, error) {
	output = bytes.TrimSpace(output)
	if len(output) == 0 {
		return securityscan.ScanReport{}, errors.New("scanner produced no JSON report")
	}

	var report securityscan.ScanReport
	if err := json.Unmarshal(output, &report); err == nil && len(report.Findings) > 0 {
		return report, nil
	}

	var finding securityscan.Finding
	if err := json.Unmarshal(output, &finding); err == nil {
		return securityscan.ScanReport{Findings: []securityscan.Finding{finding}}, nil
	}

	scanner := bufio.NewScanner(bytes.NewReader(output))
	scanner.Buffer(make([]byte, 1024), maxOutput)
	for scanner.Scan() {
		line := bytes.TrimSpace(scanner.Bytes())
		if len(line) == 0 {
			continue
		}
		var f securityscan.Finding
	...
	if err := scanner.Err(); err != nil || len(report.Findings) == 0 {
		return securityscan.ScanReport{}, errors.New("scanner emitted invalid report")
	}
	return report, nil
}
