...
The runner has a configurable `ZERRA_SCANNER_IMAGE` (default `scanner`), read-only filesystem, non-root user, no mounts, no privileged mode, PID/memory/CPU limits, and a 60-second hard timeout. The scan plan permits GET, HEAD, and OPTIONS only with an 80-request, 5-request/second budget.
